...
https://projecteuler.net/minimal=54
...
The meat of the logic is over in "euler", since it seemed like something they might ask me to use again later.  At least the card part.
...
func Euler054(fn string) (uint, uint) {
	var p1won, p2won, line uint
	fh, err := os.Open(fn)
	if nil != err {
		panic("Euler0054 unable to open: " + fn)
	}
	defer fh.Close()
	...
	// scanner.Split(bufio.ScanLines) // Already Default
	for scanner.Scan() {
		line++
		card := 0
		rawtext := scanner.Text()
		cardtext := strings.SplitN(strings.ToUpper(rawtext), " ", -1)
		cardlen := len(cardtext)
		if 10 != cardlen {
			fmt.Printf("WANRING: %s : line %d : counted %d cards : %s\n", fn, line, cardlen, rawtext)
			continue
		}
		for card < cardlen {
			hands[card] = euler.CardParseENG(cardtext[card])
			card++
		}
		...
		if p1 > p2 {
			...
			// fmt.Printf("Hand %d\t:  2 : %x %x %x %x %x (%8x)   < (%8x)\t%x %x %x %x %x\t\t%s\t\t%s\n", line, hands[0], hands[1], hands[2], hands[3], hands[4], p1, p2, hands[5], hands[6], hands[7], hands[8], hands[9], rawtext[0:14], rawtext[15:29])
			p2won++
		} else {
			fmt.Printf("INFO: %s : line %d : Scores Tied: %d == %d : %s\n", fn, line, p1, p2, rawtext)
		}
	}
	return p1won, p2won
}
...
/*
	for ii in *\/*.go ; do go fmt "$ii" ; done ; for ii in 54 ; do go fmt $(printf "pe_%04d.go" "$ii") ; go run $(printf "pe_%04d.go" "$ii") || break ; done

Euler 54: Poker Hands: 1000 played, 624 won by Player 2 and Player 1 won: 376
*/
func main() {
	//test
	// Euler054()
	...
	//run
	p1, p2 := Euler054("0054_poker.txt")
	fmt.Printf("Euler 54: Poker Hands: %d played, %d won by Player 2 and Player 1 won: %d\n", p1+p2, p2, p1)
}
